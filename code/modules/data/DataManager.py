...
class DataManager:
    ...
    def _setup(self, country):
        ...
        if country not in self._setup_countries:
            ...
            self.dataframes[country].index = self.dataframes[country].index.astype(np.int32)
            self.dataframes[country]["id"] = self.dataframes[country]["id"].astype(np.int32)
            if country == "peru":
                self.dataframes[country]["ord"] = self.dataframes[country]["ord"].astype(np.int32)

            valid = self.file_is_valid(country, directory)
            self.dataframes[country] = self.dataframes[country][valid]

            self._setup_countries.add(country)

    def file_is_valid(self, country, directory):
        dataframe = self.dataframes[country]
        filenames = self._extract_filenames(directory)
        valid = []
        for i in range(dataframe.values.shape[0]):
            if country == "kenya":
                fname = self._format_filename(dataframe["id"].iloc[i])
            elif country == "peru":
                suffix = int(dataframe.iloc[i]["ord"])
                fname = self._format_filename(dataframe["id"].iloc[i], suffix=suffix)
    ...
    def sample_class(self, dataframe, cls, n):
        ...
        replace = n > df.values.shape[0]
        return df.sample(n=n, replace=replace, random_state=self.config["seed"])
    ...
    def generate_kenya(self):
        ...
        if self.config['remove_clouds']:
            with open(os.path.join(modules.data.util.root(), "peru", "cloudy.txt"), "rt") as fd:
                cloud_filenames = [line.strip() for line in fd]
            cloud_filenames = set(map(lambda x: x[:-3] + "jpg", cloud_filenames))

            dataframe = dataframe[~dataframe['filename'].isin(cloud_filenames)]
        ...
        if self.config["sample"]:
            if not self.config["sample"]["balanced"]:
                ...
                # shuffle the data
                dataframe = dataframe.iloc[np.random.permutation(dataframe.index)]
        ...
        if self.config["mask"] == 'none':
            ...
            directory_mask = f"{modules.data.util.root()}/kenya/{self.config['image_size']}/masks/"
    ...
    def generate_peru(self):
        ...
        if self.config["sample"]:
            if not self.config["sample"]["balanced"]:
                n = min(self.config["sample"]["size"], dataframe.values.shape[0])
                dataframe = dataframe.sample(n=n, replace=False, random_state=self.config["seed"])
            else:
                dataframes_per_class = []
                for cls in self.config["class_enum"]:
...
#             shuffle=self.config["shuffle"],
    ...
    def _format_dataframe_for_flow(self, country):
        if country == 'kenya':
            return pd.DataFrame(
                list(map(
                    lambda e: (self._format_filename(e[0], suffix=None), e[1]),
                    zip(
        ...
        if country == 'peru':
            return pd.DataFrame(
                list(map(
                    lambda e: (self._format_filename(e[0], suffix=e[1]), e[2]),
                    zip(
                        map(int, self.dataframes[country]["id"]),
                        map(int, self.dataframes[country]["ord"]),
    ...
    def _format_filename(self, idx, suffix=None, ext="jpg"):
        if suffix is None:
            return f"{idx}.{ext}"
        else:
            return f"{idx}-{suffix}.{ext}"
